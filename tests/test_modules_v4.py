...
async def test_log_audit_detects_clear_audit_log():
    ...
    assert any(f.owasp_category == OWASPCategory.MCP10_LOG_AUDIT for f in flagged)
...
async def test_log_audit_clean_tools_not_flagged():
    ...
    log_flagged = {f.tool_name for f in findings if f.owasp_category == OWASPCategory.MCP10_LOG_AUDIT}
