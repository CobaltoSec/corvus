...
**Target:** `uvx arxiv-latex-mcp`
**Transport:** stdio
**Date:** 2026-07-05 20:16:36
**Duration:** 38.8s
...
### CORVUS-001 — Injection reflected — 'get_paper_prompt.arxiv_id'

| Field | Value |
|-------|-------|
| Severity | **HIGH** |
| OWASP | MCP05 |
| Tool | `get_paper_prompt` |
| Parameter | `arxiv_id` |
| Confidence | 85% |
...
**Evidence:**
```
Error processing arXiv paper <script>alert(1)</script>: unsupported operand type(s) for +: 'NoneType' and 'str'
```
...
### CORVUS-002 — Injection reflected — 'get_paper_abstract.arxiv_id'
...
**Evidence:**
```
Error processing arXiv paper <script>alert(1)</script>: 1 validation error for TextContent
text
...
### CORVUS-003 — Injection reflected — 'list_paper_sections.arxiv_id'

| Field | Value |
|-------|-------|
| Severity | **HIGH** |
| OWASP | MCP05 |
| Tool | `list_paper_sections` |
| Parameter | `arxiv_id` |
| Confidence | 85% |
...
**Evidence:**
```
Error processing arXiv paper <script>alert(1)</script>: expected string or bytes-like object, got 'NoneType'
```
...
### CORVUS-006 — Server allows external log level escalation to DEBUG
...
**Evidence:**
```
Server disconnected after batch. Pre-crash response: {"jsonrpc":"2.0","id":42,"result":{"protocolVersion":"2025-11-25","capabilities":{"experimental":{},
```
