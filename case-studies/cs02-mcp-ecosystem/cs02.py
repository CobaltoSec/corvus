#!/usr/bin/env python3
"""CS02 — MCP Ecosystem Audit CLI (Tier D community servers).

Commands:
  status                           Show target state table
  scan [--tier D] [--name N]       Run corvus batch on pending/error targets
...
Examples:
  python cs02.py status
  python cs02.py scan --tier D
  python cs02.py scan --name desktop-commander
  python cs02.py update batch-scans/20260626-120000
  python cs02.py add mcp-my-server --tier D --cmd npx -y mcp-my-server
"""
import argparse
import json
import shutil
...
def cmd_status(_args: argparse.Namespace, data: dict) -> None:
    ...
    print(f"\n{_BOLD}CS02 — MCP Ecosystem Audit (Tier D){_RESET}  "
...
def cmd_scan(args: argparse.Namespace, data: dict) -> None:
    ...
    print(f"Run 'python cs02.py status' to see results.")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="cs02",
        description="CS02 MCP Ecosystem Audit — Tier D target tracker",
    )
    ...
    p_scan.add_argument("--tier", metavar="A|B|C|D", help="Filter by tier")
    ...
    p_add.add_argument("--tier", required=True, choices=["A", "B", "C", "D"])
